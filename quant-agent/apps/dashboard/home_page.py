...
<body>
  <div class="wrap">
    ...
    <div class="stats">
      ...
      <div class="stat"><div class="k">纸交易影子</div><div class="v" id="paperShadowStatus">0/20</div></div>
  ...
  <script>
    ...
    const orderIdempotencyKeys = new Map();
    const sellIdempotencyKeys = new Map();
    ...
    async function postJson(path, payload, method = 'POST') {
      const csrfToken = cookieValue('quant_csrf');
      ...
      body.innerHTML = items.map((r, idx) => `
        <tr>
          ...
          <td>
            ...
            <a class="link" href="/dashboard/recommendations/${encodeURIComponent(r.id)}" target="_blank">查看</a>
          </td>
        </tr>
      `).join("");
    }
    ...
    function paperOrderPayload(rec, qty) {
      ...
      if (!orderIdempotencyKeys.has(rec.id)) {
        orderIdempotencyKeys.set(rec.id, `dashboard:${rec.id}:${crypto.randomUUID()}`);
      }
      return {
        recommendation_id: rec.id,
        idempotency_key: orderIdempotencyKeys.get(rec.id),
    ...
    async function sellHolding(index, sellAll) {
      ...
      const key = `${holding.ticker}:${sellAll ? 'all' : 'partial'}`;
      if (!sellIdempotencyKeys.has(key)) {
        sellIdempotencyKeys.set(key, `dashboard:sell:${holding.ticker}:${crypto.randomUUID()}`);
      }
      const payload = {
        idempotency_key: sellIdempotencyKeys.get(key),
        sell_price: price,
    ...
    async function executeAlert(index) {
      ...
      const key = `${alert.ticker}:alert:${alert.reason_code}`;
      if (!sellIdempotencyKeys.has(key)) {
        sellIdempotencyKeys.set(key, `dashboard:alert-sell:${alert.ticker}:${crypto.randomUUID()}`);
      }
      const payload = {
        idempotency_key: sellIdempotencyKeys.get(key),
    ...
    async function loadData() {
      ...
      const paperShadow = data.paper_shadow_readiness || {};
      const shadowObserved = Number(paperShadow.observed_trading_days ?? 0);
      const shadowRequired = Number(paperShadow.required_trading_days ?? 20);
      const shadowClass = paperShadow.passed ? 'b-ok' : 'b-neutral';
      const shadowStatus = document.getElementById('paperShadowStatus');
      shadowStatus.innerHTML = `<span class="badge ${shadowClass}">${esc(shadowObserved)}/${esc(shadowRequired)}</span>`;
      shadowStatus.title = paperShadow.first_trading_date
        ? `首日 ${paperShadow.first_trading_date} | 最近 ${paperShadow.last_trading_date || '-'}`
        : '尚无合格的真实交易日证据';
