        ...
        with SessionLocal() as session:
            session.merge(
                TradeLedgerRecord(
                    ...
                    strategy_config_id=entry.strategy_config_id,
                    reason=entry.reason,
...
class AutopilotPolicyRepository:
    ...
    def _to_domain(record: AutopilotPolicyRecord) -> AutopilotPolicy:
        return AutopilotPolicy(
            ...
            max_daily_auto_approvals=getattr(record, "max_daily_auto_approvals", 3),
            ...
            max_daily_realized_loss_pct=getattr(record, "max_daily_realized_loss_pct", 0.03),
            max_auto_buy_price_drift_pct=getattr(record, "max_auto_buy_price_drift_pct", 0.03),
            require_position_reconciliation=bool(getattr(record, "require_position_reconciliation", 0)),
            max_position_reconciliation_age_minutes=getattr(
                record,
