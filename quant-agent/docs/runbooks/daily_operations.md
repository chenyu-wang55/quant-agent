...
broker `equity` or `portfolio_value` for sizing plus `buying_power` to cover the
risk-plan notional for that order. Add
...
Automatic buys also pass `price_drift_gate`: before routing a buy, the worker checks
...
`account_equity_gate` before risk sizing, then `broker_buying_power_gate` before broker
submit. If Alpaca reports `trading_blocked`, `account_blocked`, or a non-active status,
the whole live automatic execution step is skipped. If broker `equity` and
`portfolio_value` are missing, live buys are skipped instead of using a stale static
balance. If the account is tradeable but buying power is missing or below the planned
...
When the broker adapter supports positions, `system_cycle` also records a fresh
position reconciliation report before the execution gate is evaluated. Use
`--disable-auto-position-reconciliation` to skip this poll or
`--position-reconciliation-qty-tolerance` to tune quantity matching. Before enabling
any real broker adapter, submit at least one broker/account position snapshot to
`POST /portfolio/reconciliation` and verify the dashboard report; any missing, extra,
or quantity-mismatched position sets `blocks_auto_execution=true`, which should be
treated as a hard stop for live automation until the ledger is corrected. Live
automatic execution always enforces this gate, even when
`require_position_reconciliation=false`. For paper automation, set
`require_position_reconciliation=true` in the autopilot policy or pass
`--require-position-reconciliation` to make the worker enforce the same gate.
...
`live_dry_run`, confirming Alpaca paper-trading credentials, and confirming the latest
broker position reconciliation is `matched` or `empty`. The service still needs the
broker account snapshot to show an active, unblocked account with enough buying power
for each automatic buy. It also needs
...
latest price is too far from the recommendation entry zone.
Live automatic execution is skipped unless the latest `position_reconciliations`
report is `matched` or `empty` and no older than
`max_position_reconciliation_age_minutes`. Paper automation enforces the same rule
when `require_position_reconciliation` is true.

The automatic broker position poll uses the same reconciliation model. You can still
record a manual broker reconciliation snapshot before live cutover or after any manual
broker action:

```bash
curl -X POST http://localhost:8000/portfolio/reconciliation \
  -H "Content-Type: application/json" \
  -d '{
    "broker": "manual",
    "account_id": "paper-account",
    "positions": [
      {"ticker": "AAPL", "qty": 10, "avg_price": 180.25}
    ],
    "note": "pre-live reconciliation"
  }'
```
