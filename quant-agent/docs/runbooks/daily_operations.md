...
python -m apps.worker.main system_cycle_loop --interval-seconds 300 --auto-execute-approved --auto-execution-mode paper
```
...
`auto_approval` and `auto_execution` and are also persisted in the run metrics JSON.
