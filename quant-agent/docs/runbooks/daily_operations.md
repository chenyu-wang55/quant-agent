...
`blocks_auto_execution=true`, which should be treated as a hard stop for live
automation until the ledger is corrected. Set `require_position_reconciliation=true`
in the autopilot policy or pass `--require-position-reconciliation` to make the worker
enforce this as an automatic execution gate.
...
Record a broker reconciliation snapshot before live cutover or after any manual broker
action:
