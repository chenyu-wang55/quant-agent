...
treated as a hard stop for live automation until the ledger is corrected. Set
`require_position_reconciliation=true` in the autopilot policy or pass
`--require-position-reconciliation` to make the worker enforce this as an automatic
execution gate.
...
When `require_position_reconciliation` is true, automatic execution is skipped unless
the latest `position_reconciliations` report is `matched` or `empty` and no older than
